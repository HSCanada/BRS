...
ALTER VIEW [Fact].Sales_ds
AS

/******************************************************************************
**	File: 
**	Name: [Fact].Sales_ds
**	Desc:  used for Daily Sales Cube (BI backend)
**		
**
**              
**	Return values:  
...
FROM         
	...
	INNER JOIN [dbo].[BRS_AdjCode] adj_code
	ON t.AdjCode = adj_code.AdjCode

WHERE 
	-- Enabled when IN monthend mode
	t.FiscalMonth = 202509 AND 
	(1=1)
...
select top 10 * from [Fact].Sales_ds
GO
