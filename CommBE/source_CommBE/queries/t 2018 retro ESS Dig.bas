...
Where ="(((comm_transaction.fiscal_yearmo_num) Between \"201701\" And \"201712\") AND (("
    "comm_transaction.ess_comm_plan_id) Like \"ccs*\" Or (comm_transaction.ess_comm_p"
    "lan_id) Like \"ess*\") AND ((comm_transaction.ess_comm_group_cd)<>[comm_group_cd"
    "]) AND ((comm_item_master.comm_group_cd) Like \"dig*\")) OR (((comm_transaction."
    "fiscal_yearmo_num) Between \"201701\" And \"201712\") AND ((comm_transaction.ess"
    "_comm_plan_id) Like \"ccs*\" Or (comm_transaction.ess_comm_plan_id) Like \"ess*\""
    ") AND ((comm_transaction.ess_comm_group_cd) Like \"dig*\") AND ((comm_item_maste"
    "r.comm_group_cd)<>[ess_comm_group_cd]))"
Begin InputTables
